...
export type Product = {
  ...
  category: string;
  stock: number;
  /** Higher = newer. Used for "New Arrivals first" sort. */
  addedAt: number;
};
...
const badgeFor = (p: Product) => {
  ...
  if (p.addedAt >= 5) return { label: "NEW", className: "bg-primary-hi text-primary-foreground" };
  return null;
};

export const ProductGrid = ({ products, onAdd }: { products: Product[]; onAdd: (p: Product) => void }) => {
  const [cat, setCat] = useState("All");

  const filtered = useMemo(() => {
    const list = cat === "All" ? products : products.filter(p => p.category === cat);
    // New Arrivals first (highest addedAt), sold out pushed to end
            ...
            <article key={p.id} className="group border border-border bg-card hover:border-primary-hi transition-colors flex flex-col">
              ...
              <div className="p-5 flex flex-col gap-3">
                ...
                <button
                  disabled={soldOut}
                  onClick={() => onAdd(p)}
                  className="mt-2 self-start px-4 py-2.5 border border-primary text-primary text-[10px] tracking-[0.22em] uppercase hover:bg-primary hover:text-primary-foreground transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-primary"
                >
